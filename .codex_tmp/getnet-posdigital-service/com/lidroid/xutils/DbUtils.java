...
public class DbUtils {
    ...
    protected DbUtils(DaoConfig config) {
        if (config == null) {
    ...
    private SQLiteDatabase createDatabase(DaoConfig config) {
        ...
        if (!TextUtils.isEmpty(dbDir)) {
            ...
            if (dir.exists() || dir.mkdirs()) {
                 ...
                 * 首次创建数据库报错
                 * Unable to create application com.yygou.AppYygou: android.database.sqlite.SQLiteException: Failed to change locale for db '/storage/sdcard0/xxxx/xxxxx.db' to 'zh_CN'.
                 * 根据http://stackoverflow.com/questions/19491675/failed-to-change-locale-for-db-data-data-my-easymedi-controller-databases-easy
                 * 的解决思路
                 * 以及openOrCreateDatabase 方法的注释里 Equivalent to openDatabase(file.getPath(), factory, CREATE_IF_NECESSARY).
                 *  改进
                 * **/
                result = SQLiteDatabase.openDatabase(dbFile.getPath(), null, SQLiteDatabase.CREATE_IF_NECESSARY | SQLiteDatabase.NO_LOCALIZED_COLLATORS | SQLiteDatabase.OPEN_READWRITE);
            }
        } else {
