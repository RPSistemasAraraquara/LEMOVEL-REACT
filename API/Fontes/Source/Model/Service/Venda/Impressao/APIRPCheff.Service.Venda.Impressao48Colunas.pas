unit APIRPCheff.Service.Venda.Impressao48Colunas;
...
type
  TAPIRPCheffServiceVendaImpressao48Colunas = class(TAPIRPCheffServiceVendaImpressao)
  private
    FData          : TDateTime;
...
{ TAPIRPCheffServiceVendaImpressao48Colunas }

procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarEmpresa;
begin
  FreeAndNil(FEmpresa);
  ...
  FImpressao := '</zera>' + sLineBreak +
    '<n><e>' + FEmpresa.nome + '</n></e>' + sLineBreak +
    AcertaTexto(FEmpresa.endereco, 'E', 40) +
...
procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarOperadorPreFechamento;
var
...
begin
  ...
  try
    ...
    begin
      FImpressao := FImpressao + '</linha_simples>' + '</ae>' + sLineBreak +
        'Solicitador por  : '+ QuebraLinhaItemCupom(LOperadorFechamento.nome, 48) + sLineBreak;
    end;
  finally
...
procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarGarcons;
var
  ...
  try
    ...
    begin
      FImpressao := FImpressao +
        ...
        '<n>' + QuebraLinhaItemCupom(LTextoGarcom, 48) + '</n>' + sLineBreak;
    end;
  finally
    LGarcons.Free;
  end;
...
procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarItemFracionado;
var
  LPossuiFracionado: Boolean;
  descricaoTamanhoFormatado: string;
begin
  ...
  begin
    ...
    begin
      descricaoTamanhoFormatado := IfThen(LFracao.DescricaoTamanho.Trim <> '', ' (' + LFracao.DescricaoTamanho + ')', '');


      FImpressao := FImpressao + QuebraLinhaItemCupom(LFracao.produtoDescricao+descricaoTamanhoFormatado, 23, '',
    ...
    for var LOpcional in LItem.opcionais do
      FImpressao := FImpressao + '<n>' +
        QuebraLinhaItemCupom('  Opc:'+ LOpcional.descricao, 44, '    ', '') +
        '</n>' + sLineBreak + sLineBreak;
  end;
end;


procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarItemNormal(AItem: TAPIRPCheffEntityVendaItem);
var
...
begin
  ...
  FImpressao := FImpressao + QuebraLinhaItemCupom(AItem.produtoDescricao + LDescricaoTamanhoFormatado, 23, '',
...
procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarItens;
begin
  FImpressao := FImpressao +
...
procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarOpcionais(AItem: TAPIRPCheffEntityVendaItem);
var
...
procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarPagamentos;
var
...
begin
  ...
  try
    ...
    begin
      ...
      FImpressao := FImpressao + '<n>' +
        AcertaTexto(LEncerramento.formaPagamento.descricao, 'E', 39) +
        AcertaTexto(Format('%.2f',[LEncerramento.ValorPago]), 'D', 9) + '</n>' + sLineBreak;
    end;
    if LTotalTroco > 0 then
      FImpressao := FImpressao + '<n>' +
        AcertaTexto('TROCO', 'E', 39) +
        AcertaTexto(Format('%.2f',[LTotalTroco]), 'D', 9) + '</n>' + sLineBreak;

    if FValorPendente > 0 then
      FImpressao := FImpressao + '<n>' +
        AcertaTexto('VALOR PENDENTE', 'E', 39) +
        AcertaTexto(Format('R$ %.2f',[FValorPendente]), 'D', 9) + '</n>' + sLineBreak;
  finally
    LEncerramentos.Free;
  end;
end;

procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarPagamentosAntecipados;
var
...
begin
  ...
  try
    ...
    begin
      ...
      FImpressao := FImpressao + '<n>' +
        AcertaTexto(LPagamento.formaPagamento.descricao, 'E', 39) +
...
procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarRodape;
begin
  FImpressao := FImpressao + '</linha_simples>' + sLineBreak +
    '</ce>RP Cheff www.sistemalechef.com.br' + sLineBreak +
...
procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarTotal;
var
...
begin
  ...
  if LDesconto > 0 then
    FImpressao := FImpressao +
      '    Desconto nos itens: ' + Format('R$ %.2f',  [LDesconto]) + sLineBreak;

  if FEmpresa.casaNoturna then
  begin
    FImpressao := FImpressao + AcertaTexto('Taxa Entrada (+)', 'E', 38, '.') + ':' +
      AcertaTexto(Format('R$ %.2f', [FVenda.valorEntrada]), 'D', 9) + sLineBreak;

    FImpressao := FImpressao + 
      AcertaTexto('Taxa Cart'#227'o (+)', 'E', 38, '.') + ':' +
      AcertaTexto(Format('R$ %.2f', [FVenda.taxaCartao]), 'D', 9) + sLineBreak +
      AcertaTexto('Taxa de servi'#231'o (+)', 'E', 38, '.') + ':' +
      AcertaTexto(Format('R$ %.2f', [FVenda.valorTaxaServico]), 'D', 9) + sLineBreak + sLineBreak +
      '<n><e>TOTAL CONTA' + AcertaTexto(Format('R$ %.2f', [FVenda.valorTotal]), 'D', 13) +
  ...
  begin
    if FVenda.valorTaxaServico > 0 then
      FImpressao := FImpressao + AcertaTexto('Taxa de servi'#231'o (+)', 'E', 38, '.') + ':' +
        AcertaTexto(Format('%.2f', [FVenda.valorTaxaServico]), 'D', 9) + sLineBreak;

    if FVenda.ValorCouvert > 0 then
      FImpressao := FImpressao + AcertaTexto(FMensagemCouvert, 'E', 38, '.') + ':' +
...
procedure TAPIRPCheffServiceVendaImpressao48Colunas.CarregarVenda;
var
...
begin
  ...
  try
    ...
    FImpressao := FImpressao + '<n><e></ae>' + FVenda.DescricaoMesaComanda + ' - Cupom: ' + FVenda.numeroCupom.ToString +
      ...
      '         CUPOM PARA SIMPLES CONFERENCIA ' + sLineBreak +
      '</linha_simples>' + sLineBreak;
  finally
...
destructor TAPIRPCheffServiceVendaImpressao48Colunas.Destroy;
begin
  FVenda.Free;
  FEmpresa.Free;
  FVendaItens.Free;
...
function TAPIRPCheffServiceVendaImpressao48Colunas.Execute: TStream;
begin
  FImpressao := EmptyStr;
  FData := Now;
  CarregarEmpresa;
